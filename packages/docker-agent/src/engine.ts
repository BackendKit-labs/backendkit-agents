...
// Agents
import { DOCKER_ORCHESTRATOR_PROFILE } from './agents/infra-orchestrator';
...
export function createDockerEngine(transport: CallbackTransport): AgentEngine {
  ...
  toolRegistry
    ...
    .register(imagePull).register(imageBuild).register(imageList).register(imageRemove).register(imageTag).register(imagePush)
    .register(networkCreate).register(networkList).register(networkInspect).register(networkRemove)
    .register(networkConnect).register(networkDisconnect).register(networkPrune).register(networkDiagnose).register(networkDnsLookup)
    .register(composeTool.up).register(composeTool.down).register(composeTool.build).register(composeTool.ps).register(composeTool.logs)
    .register(volumeCreate).register(volumeList).register(volumeInspect).register(volumeRemove)
    .register(swarmServiceCreate).register(swarmServiceList).register(swarmServiceInspect).register(swarmServiceLogs)
    .register(swarmServiceUpdate).register(swarmServiceRemove).register(swarmStackDeploy).register(swarmStackList)
    .register(swarmStackRemove).register(swarmNodeList)
    .register(containerdRun).register(containerdList).register(containerdStop).register(containerdRemove).register(containerdLogs).register(containerdPull)
  ...
  agentRegistry
    .register(DOCKER_ORCHESTRATOR_PROFILE)
  ...
  return new AgentEngine({
    ...
    defaultAgentId: 'docker-orchestrator',
    transport,
    maxIterations: 50,
    iterationMode: 'auto',
  });
