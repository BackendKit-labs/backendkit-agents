...
import { ConfigStore, type StoredAgent, type StoredFlow } from './config-store.js';
...
function isGate(r: FlowResult | GateHit | StepFailed): r is GateHit       { return (r as GateHit).gateRequired === true; }
function isStepFailed(r: FlowResult | GateHit | StepFailed): r is StepFailed { return (r as StepFailed).stepFailed === true; }

// ── Flow loading from ConfigStore ─────────────────────────────────────────────

function loadFlowFromStore(cs: ConfigStore, flowId: string) {
    const sf = cs.getFlow(flowId);
    if (!sf) throw new Error(`Flow '${flowId}' not found in config store`);
    return ConfigStore.toExecutorFlow(sf);
}

function matchFlowFromStore(cs: ConfigStore, task: string): { id: string; sf: StoredFlow } | null {
    for (const sf of cs.listFlows()) {
        if (sf.trigger && new RegExp(sf.trigger, 'i').test(task)) return { id: sf.id, sf };
    }
    return null;
}
...
async function executeAndPersist(
    ...
    const { executor, store, configStore } = ctx;
    const flow = loadFlowFromStore(configStore, flowId);
...
export async function hRunFlow(ctx: HandlerCtx, flowId: string, input: Record<string, unknown>): Promise<string> {
    if (!ctx.configStore.getFlow(flowId)) {
        const ids = ctx.configStore.listFlows().map(f => f.id).join(', ');
        return `Flow '${flowId}' not found. Available: ${ids || '(none)'}`;
    }
    return executeAndPersist(ctx, flowId, input);
}

export async function hRunTask(ctx: HandlerCtx, task: string, input: Record<string, unknown>): Promise<string> {
    const match = matchFlowFromStore(ctx.configStore, task);
    if (!match) {
        const flows = ctx.configStore.listFlows();
        return [
            `No flow matched task: "${task}"`,
            `Available: ${flows.map(f => `${f.id} (trigger: ${f.trigger ?? 'manual'})`).join(', ') || '(none)'}`,
        ].join('\n');
    }
    return executeAndPersist(ctx, match.id, { ...input, task });
}
...
export async function hApprove(ctx: HandlerCtx, runId: string, feedback?: string): Promise<string> {
    ...
    try {
        ...
        if (!configStore.getFlow(run.flowId)) return `Flow '${run.flowId}' not found in config`;

        Object.assign(run, { status: 'running', gateStepId: undefined, updatedAt: ts() });
        await store.save(run);

        const flow  = loadFlowFromStore(configStore, run.flowId);
        ...
        try {
            ...
            if (isGate(result)) {
                Object.assign(run, { status: 'waiting_gate', currentStepId: undefined, results: result.completedSoFar, gateStepId: result.stepId, gateCriteria: result.criteria, updatedAt: ts() });
                await store.save(run);
                return [`⏸ Next gate at step "${result.stepId}"`, `\nOutput:\n${result.output}`, `\nApprove: orchestrator_approve(run_id="${runId}")`].join('\n');
            }
            if (isStepFailed(result)) {
                Object.assign(run, { status: 'waiting_retry', currentStepId: undefined, results: result.completedSoFar, failedStepId: result.stepId, error: result.error, updatedAt: ts() });
                await store.save(run);
                return [`✗ Required step "${result.stepId}" failed`, `\nError: ${result.error}`, `\nRetry: orchestrator_retry(run_id="${runId}")`].join('\n');
            }
    ...
    try {
        ...
        try {
            ...
            if (isGate(result)) {
                Object.assign(run, { status: 'waiting_gate', currentStepId: undefined, results: result.completedSoFar, gateStepId: result.stepId, gateCriteria: result.criteria, updatedAt: ts() });
                await store.save(run);
                return [`⏸ Gate at step "${result.stepId}"`, `\nApprove: orchestrator_approve(run_id="${runId}")`].join('\n');
            }
...
export async function hStatus(ctx: HandlerCtx, runId: string): Promise<string> {
    ...
    if (!run) return `Run '${runId}' not found`;
    return [
        `Run:    ${run.id}`, `Flow:   ${run.flowId}`, `Status: ${run.status}`,
...
export async function hListAgentsJson(ctx: HandlerCtx): Promise<string> {
    ...
    const agents = ctx.registry.agentIds().map(id => {
        const cfg = ctx.registry.config(id);
        return {
            id, description: cfg.description ?? '', strategy: cfg.strategy,
...
export async function hListRuns(ctx: HandlerCtx): Promise<string> {
    return JSON.stringify(await ctx.store.list());
}

export async function hGetConfig(ctx: HandlerCtx): Promise<string> {
    const { configStore, registry, config } = ctx;
    const agents = configStore.listAgents();
    const flows  = configStore.listFlows();

    const health = await registry.healthAll();

    return JSON.stringify({
        orchestrator: { name: config.orchestrator.name, data_dir: config.orchestrator.data_dir },
        agents: agents.map(a => ({
            id: a.id, description: a.description, strategy: a.strategy, capability: a.capability,
            timeout: a.timeout, gate: a.gate, gate_criteria: a.gate_criteria,
            instances: a.instances.map(i => ({
                url: i.url,
                ok:  (health[a.id] ?? []).find(h => h.url === i.url)?.ok ?? false,
            })),
        })),
        flows: flows.map(f => ({
            id: f.id, name: f.name, description: f.description, trigger: f.trigger, steps: f.steps,
        })),
        settings: { maxConcurrentFlows: Number(process.env['MAX_CONCURRENT_FLOWS'] ?? 3) },
    });
}

// ── CRUD handlers ─────────────────────────────────────────────────────────────

export async function hSaveAgent(ctx: HandlerCtx, agent: StoredAgent): Promise<string> {
    ctx.configStore.saveAgent(agent);
    try { ctx.registry.upsertAgent(agent); } catch (e) { return JSON.stringify({ ok: false, error: (e as Error).message }); }
    return JSON.stringify({ ok: true, id: agent.id });
}

export async function hDeleteAgent(ctx: HandlerCtx, id: string): Promise<string> {
    ctx.configStore.deleteAgent(id);
    ctx.registry.removeAgent(id);
    return JSON.stringify({ ok: true, id });
}

export async function hSaveFlow(ctx: HandlerCtx, flow: StoredFlow): Promise<string> {
    ctx.configStore.saveFlow(flow);
    return JSON.stringify({ ok: true, id: flow.id });
}

export async function hDeleteFlow(ctx: HandlerCtx, id: string): Promise<string> {
    ctx.configStore.deleteFlow(id);
    return JSON.stringify({ ok: true, id });
}
