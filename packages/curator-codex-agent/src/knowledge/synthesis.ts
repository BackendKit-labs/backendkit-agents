 ...
 * Generates synthetic notes from search results and base notes
 */
...
export class KnowledgeSynthesizer {
    ...
    /**
     * Generate a synthetic note from search results
     */
    async synthesize(
        query: string,
        ...
        const prompt = SYNTHESIS_PROMPT.replace('{query}', query).replace(
            '{documents}',
            documentsText
        );
    ...
    /**
     * Save synthesized note to vault
     */
