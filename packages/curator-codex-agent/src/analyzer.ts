...
// Zod schema for code analysis response
  ...
  "notes": [
    {
      ...
      "resumen": string,         // 1-2 sentences DENSE with key terms, types, functions.
      "content": string,         // Markdown with ## sections. Extract APIs, types, examples.
      "tags": string[],          // e.g. ["code/typescript", "modulo/auth", "patron/jwt"]
      "language": string,        // typescript, javascript, python, go, rust, etc
      "files": string[],         // Relative paths of analyzed files
      "depends_on": string[],    // External dependencies or modules
      "exports": string[],       // Public APIs, functions, classes
      "version": number          // optional, if versioned
    }
  ]
}
...
function buildFrontmatter(note: CodeAnalysisNote, source: string, date: string, sourcesCombined?: string[]): string {
    ...
    if (sourcesCombined && sourcesCombined.length > 0) {
        const sources = sourcesCombined.map(s => `"${s}"`).join(', ');
        lines.push(`sources_combined: [${sources}]`);
    }
...
export class CodeAnalyzer {
    ...
    constructor(opts: CodeAnalyzerOptions) {
        ...
        this.docCurator = new DocumentationCurator({
    ...
    async findAssociatedDocs(
        ...
        // 1. Search for .md with same name
        const mdFile = allFiles.find(f =>
            f.fullPath === path.join(dirname, `${basename}.md`)
        );
        if (mdFile) result.docFile = mdFile.fullPath;

        // 2. Search for README.md in same directory
        const readmeFile = allFiles.find(f =>
            f.fullPath === path.join(dirname, 'README.md')
        );
        if (readmeFile) result.readmeFile = readmeFile.fullPath;

        // 3. Search in docs/ folder with similar name
        if (!result.docFile) {
        ...
        // 4. Search for root README.md
        if (!result.contextFile) {
            const globalReadme = allFiles.find(f =>
    ...
    async analyzeFile(
        ...
        if (this.isDocFile(filePath)) {
            return await this.analyzeDocumentation(filePath, relativePath);
        } else {
    ...
    private async analyzeCode(
        ...
        if (allFiles) {
            ...
            if (associated.docFile) {
                try {
                    context.docContent = await fs.readFile(associated.docFile, 'utf-8');
                } catch {
                    // ignore if can't read doc file
                }
            }
            if (associated.contextFile) {
                try {
                    context.contextContent = await fs.readFile(associated.contextFile, 'utf-8');
                } catch {
                    // ignore if can't read context file
                }
            }
            if (associated.readmeFile) {
                try {
                    context.readmeContent = await fs.readFile(associated.readmeFile, 'utf-8');
                } catch {
                    // ignore if can't read readme
                }
            }
        }
     ...
     */
    private async analyzeDocumentation(filePath: string, relativePath: string): Promise<CodeAnalysisResult> {
    ...
    private isDocFile(filePath: string): boolean {
        return ['.md', '.txt'].some(ext => filePath.toLowerCase().endsWith(ext));
    }
    ...
    private getSourcesCombined(
        ...
        // Add associated doc
        const docFile = allFiles.find(f =>
            f.fullPath === path.join(dirname, `${basename}.md`)
        );
        if (docFile) sources.push(docFile.relativePath);

        // Add README if in same dir
        const readmeFile = allFiles.find(f =>
            f.fullPath === path.join(dirname, 'README.md')
        );
